...
/**
 * Envía un correo de recordatorio de cobro de préstamo
 * @param {Object} loan - Datos del préstamo
 */
async function sendLoanReminderNotification(loan) {
  const apiKey = process.env.BREVO_API_KEY;
  const fromEmail = process.env.EMAIL_FROM;
  const fromName = process.env.EMAIL_FROM_NAME;
  ...
  const formattedAmount = `S/. ${parseFloat(loan.amount).toFixed(2)}`;
  const formattedDebt = `S/. ${parseFloat(loan.remaining_debt).toFixed(2)}`;
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Recordatorio de Cobro - Préstamo</title>
      <style>
        body {
          font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
          background-color: #f5f5f7;
        ...
        .badge {
          ...
          border-radius: 100px;
          margin-bottom: 16px;
          border: 1px solid rgba(183, 94, 0, 0.2);
        }
        .amount-display {
          font-size: 36px;
          font-weight: 700;
    ...
    <body>
      <div class="container">
        <div class="header">
          <div class="logo">GastosAhorro</div>
          <div class="subtitle">Módulo de Préstamos</div>
        </div>
        
        <div style="text-align: center;">
          <span class="badge">RECORDATORIO DE COBRO</span>
        </div>
        ...
        <div class="card">
          <div class="row">
            <span class="label">Deudor</span>
            <span class="value">${loan.borrower_name}</span>
          </div>
          <div class="row">
          ...
          <div class="row">
            ...
            <span class="value">${loan.description}</span>
          </div>
          <div class="row">
            <span class="label">Fecha del Préstamo</span>
            <span class="value">${loan.loan_date}</span>
          </div>
          <div class="row">
            <span class="label">Fecha de Vencimiento</span>
            <span class="value" style="color: #ff3b30;">${loan.due_date}</span>
          </div>
          <div class="row">
            <span class="label">Registrado por</span>
            <span class="value">${loan.spender_name}</span>
          </div>
        </div>
        ...
        <div class="footer">
          Este correo de recordatorio fue enviado automáticamente por GastosAhorro.<br>
          ¡Mantengan el control de sus finanzas! ⚖️
        </div>
      </div>
    </body>
    </html>
  ...
  const requestBody = {
    ...
    subject: `⚠️ Recordatorio de Cobro: Préstamo a ${loan.borrower_name} vence hoy o ya venció`,
    htmlContent: htmlContent
  };
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json();
      console.log('Recordatorio de préstamo enviado con éxito:', data);
      return { success: true, messageId: data.messageId };
    } else {
      const errorText = await response.text();
      console.error('Error al enviar recordatorio de préstamo:', errorText);
      return { success: false, error: errorText };
    }
  } catch (error) {
    console.error('Error de red al enviar recordatorio de préstamo a Brevo:', error);
    return { success: false, error: error.message };
  }
}
